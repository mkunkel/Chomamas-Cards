...
exports.findGame = function(name, fn){
  Game.findOne({name:name}).populate('players').populate('reader').exec(function(err, game){
    console.log(fn);
    fn(err, game);
  });
};
...
exports.populateHand = function(player, game, fn) {
  for(var i = 0; i < 7; i++) {
    var card = game.whiteCards.pop();
    player.whiteCards.push(card);
  }
